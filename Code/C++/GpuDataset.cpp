...
GpuDataset::GpuDataset(size_t totalPixels, size_t inputChannels, size_t targetChannels)
{
    ...
    m_pixelBytes = totalPixels * sizeof(engineFloat);
    ...
    CUDA_CHECK(cudaMalloc((void**)&d_pixelX, m_pixelBytes));
    CUDA_CHECK(cudaMalloc((void**)&d_pixelY, m_pixelBytes));
    ...
    const std::vector<engineFloat>& pixelX, const std::vector<engineFloat>& pixelY)
{
    ...
    CUDA_CHECK(cudaMemcpy(d_pixelX, pixelX.data(), m_pixelBytes, cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_pixelY, pixelY.data(), m_pixelBytes, cudaMemcpyHostToDevice));

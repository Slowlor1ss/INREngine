...
inline void ParseCommandLine(const int argc, char** argv)
{
...
inline std::vector<float> PositionalEncode(float x, float y, int numFrequencies) {
...
inline std::string GetCheckpointFilename(const std::string& imageFilename, const std::string& outbasePath = "")
{
	namespace fs = std::filesystem;
...
inline void LoadCheckpoint(Network& network, const std::string& filename)
{
...
inline void SaveCheckpoint(Network& network, const std::string& filename)
{
...
inline UserAction PollUserAction()
{
...
inline bool HandleUserAction(UserAction action, Network& network, const std::string& weightsFile, bool& liveUpdateWindow)
{
	switch (action)
	{
...
inline size_t GetRandomImageIndex(const size_t totalImages)
{
...
inline float RunTrainingEpoch(Network& network,
...
inline void LoadMNISTData(std::vector<std::vector<float>>& trainImages,
